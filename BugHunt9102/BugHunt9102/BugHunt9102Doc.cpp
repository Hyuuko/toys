...
CBugHunt9102Doc::CBugHunt9102Doc() noexcept
{
    ...
    for (int i = 0; i != 5; ++i)
    {
        Bug* pBug = new FastBug(IDB_BLUEBUG, 4, 18, 3, 1, 0.5);
        m_listBug.push_back(pBug);
    }
    // 5个SlowBug
    for (int i = 0; i != 5; ++i)
    {
        Bug* pBug = new SlowBug(IDB_REDBUG, 4, 18, 5, 1, 0.5);
        m_listBug.push_back(pBug);
    }
...
// 加一个FastBug
void CBugHunt9102Doc::AddBlueBug()
{
    Bug* pBug = new FastBug(IDB_BLUEBUG, 4, 18, 3, 1, 0.5);
    m_listBug.push_back(pBug);
}

void CBugHunt9102Doc::AddRedBug()
{
    Bug* pBug = new SlowBug(IDB_REDBUG, 4, 18, 5, 1, 0.5);
    m_listBug.push_back(pBug);
}
